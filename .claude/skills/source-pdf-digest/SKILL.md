...
### D. 본문 서술 (OpenAI) + 검증 (analyst)
**서술은 OpenAI(ChatGPT), 검증은 Claude analyst.** 역할 분담:
1. **OpenAI 서술 (메인 세션)** — 오케스트레이터가 `python3 scripts/summarize_openai.py {workdir}` 실행. 검증된 `03_dossier.json`(+`02_selection.json`)을 근거로 **개요·주요내용·시사점** 본문을 한글 서술해 `04_analysis.json`을 만든다.
   - 모델: env `OPENAI_MODEL`(기본 `gpt-4o`). 키: env `OPENAI_API_KEY`(필수). 출처·단신 링크는 스크립트가 dossier에서 그대로 옮겨 정확성 보장(LLM 미경유).
   - **키 부재/실패 시 exit≠0** → analyst가 동일 스키마로 직접 작성(폴백).
2. **analyst 검증** — `04_analysis.json`의 수치·고유명사·주장이 모두 dossier에 근거하는지 대조해 환각·과장을 교정한다. `topic_kind`에 맞는 관점이 ③ 시사점에 반영됐는지 확인. 작성 기준·관점·분량은 `references/ranking-and-analysis.md` 참조.

### E. PDF 조립 (analyst)
1. `assets/brief_template.html`을 복사해 Keynote 박스 + 본문 3섹션(개요·주요내용·시사점) + 출처 + 푸터(그 외 주목 단신)를 채워 `{workdir}/05_digest.html`을 만든다. 대표 이미지는 **로컬 다운로드 후 임베드**(핫링크 금지).
...
- [ ] OpenAI 서술의 모든 수치·주장이 dossier에 근거하는가(환각 교정 완료)
- [ ] 모든 주장에 원천 직링크가 있는가
...
- OpenAI 본문 서술: `scripts/summarize_openai.py` (env `OPENAI_API_KEY`·`OPENAI_MODEL`)
