...
1. **GPT-5.5 초안 (메인 세션)** — 오케스트레이터가 `python3 scripts/summarize_openai.py {workdir}` 실행. `03_dossier.json`(+`02_selection.json`) 근거로 **무엇이 달라졌나·사실과 쟁점·왜 중요한가**를 한글 서술해 `04_analysis.json`을 만든다.
...
4. **반영·확정 (Opus 4.8 = analyst)** — GPT-5.5 지적 중 dossier 근거로 타당한 것을 반영(부당하면 기각), 최종 `04_analysis.json` 확정(`generator`=`"GPT-5.5 draft × Opus 4.8 verify"`). `topic_kind`에 맞는 관점이 ③ 왜 중요한가에 반영됐는지 확인. 기준·관점·분량은 `references/ranking-and-analysis.md` 참조.

### E. PDF 조립 (analyst) — 시각 강화 템플릿 v2
1. `assets/brief_template.html`(본문 11pt·인포그래픽 내장)을 복사해 채운다: 마스트헤드 → 헤드라인+키워드 배지 → **KPI 스탯카드 3~4개** → 한눈에 보기 → ① 무엇이 달라졌나 → **벤치마크 바차트** → ② 사실과 쟁점(+대표 이미지 플로팅) → ③ 왜 중요한가(관점 박스) → ④ 출처 → 단신 → 푸터.
...
- [ ] ② 사실과 쟁점에 연구자 수준 디테일(수치·방법·벤치마크)과 쟁점·미해결 질문이 있는가
- [ ] ③ 왜 중요한가가 주제에 맞는 관점으로 분석되고 향후 관전 포인트까지 담았는가(주관점 명시)
