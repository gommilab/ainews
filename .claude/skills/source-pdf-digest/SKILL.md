...
`03_dossier.json`을 기반으로 **deep-research 방법론을 1건으로 한정 적용**: 팬아웃 검색으로 사실·수치를 교차검증하고, 출처가 부족하면 보강한다. 7섹션 분석을 `04_analysis.json`으로 작성한다. 섹션별 작성 기준·주제 적응형 관점·분량 캘리브레이션은 `references/ranking-and-analysis.md` 참조.

### E. PDF 조립 (analyst)
1. `assets/brief_template.html`을 복사해 7섹션 + 푸터(그 외 주목 단신)를 채워 `{workdir}/05_digest.html`을 만든다. 대표 이미지는 **로컬 다운로드 후 임베드**(핫링크 금지).
...
- [ ] 기술 핵심에 연구자 수준 디테일(수치·방법·벤치마크)이 있는가
- [ ] 함의가 주제에 맞는 관점으로 분석됐는가(주관점 명시)
- [ ] 모든 주장에 원천 직링크가 있는가
